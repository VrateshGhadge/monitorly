...
function IncidentBars({
  ...
  const slot = 600 / n;
  const barHeight = baseline - 8; // all bars share the same height
  ...
  return (
    ...
    >
      {points.map((p, i) => {
        ...
        const y = baseline - barHeight;
        const active = hoverIndex === i;
        const fill = isDown ? "var(--red)" : "var(--green)";
        return (
          <rect
            key={i}
            x={x}
...
function SingleRowUptimeHistory({
  ...
  const minTime =
    points.length > 0
      ? parseTimestamp(points[0].timestamp)
      : Date.now() - 24 * 3600 * 1000;
  const maxTime =
    points.length > 0
      ? parseTimestamp(points[points.length - 1].timestamp)
      : Date.now();
  const axisLabels = useMemo(
    () => getAxisLabels(minTime, maxTime, range),
      ...
      <div className="incident-graph">
        ...
        {hoverPoint && (
          <div
            ...
            style={{
              left: `${(hoverIndex! / points.length) * 100}%`,
              top: "6%",
            }}
          >
...
function MultiRowUptimeHistory({
  ...
  const minTime =
    firstPoints.length > 0
      ? parseTimestamp(firstPoints[0].timestamp)
      : Date.now() - 24 * 3600 * 1000;
  const maxTime =
    firstPoints.length > 0
      ? parseTimestamp(firstPoints[firstPoints.length - 1].timestamp)
      : Date.now();
  const axisLabels = useMemo(
    () => getAxisLabels(minTime, maxTime, range),
    [minTime, maxTime, range],
          ...
          return (
            ...
            >
              <div className="uptime-row-head">
                ...
                <span>
                  {row.flagged > 0
                    ? `${row.flagged} alert${row.flagged === 1 ? "" : "s"}`
                    : row.status === "down"
                      ? "Down now"
                      : "100% uptime"}
                </span>
              ...
              <div className="incident-graph uptime-row-graph">
                <IncidentBars
                  ...
                  hoverIndex={hoverIdx}
                  onHoverIndex={(idx) =>
                    setHoverIndex(idx === null ? null : { id: row.id, idx })
                  }
                  height={40}
                  baseline={34}
                />
                {hoverIdx !== null && row.points[hoverIdx] && (
                  <div
                    className="graph-tooltip"
                    style={{
                      left: `${(hoverIdx / row.points.length) * 100}%`,
                      top: "0%",
                    }}
                  >
                    {new Date(
                      parseTimestamp(row.points[hoverIdx].timestamp),
                    ...
                    <b>
                      {row.points[hoverIdx].status === "DOWN"
                        ? " · Down"
                        : " · Up"}
                    </b>
                  </div>
...
//  * hook state.
//  */
// export default function UptimeHistoryGraph({ live = false, monitors, maxRows = 2, onViewAll }: UptimeHistoryGraphProps) {
